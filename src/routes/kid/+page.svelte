<script lang="ts">
	...
	import { getUser, getProfile } from '$lib/stores/auth.svelte';
	import type { Kid, Transaction } from '$lib/supabase';

	let kidData: Kid | null = null;
		...
		try {
			...
			if (kidError) {
				if (kidError.code === 'PGRST116') {
					// No kid record found, create one
					const { data: newKid, error: insertError } = await supabase
						.from('kids')
						.insert({
							user_id: currentUser.id,
							name: getProfile()?.full_name || 'Kid',
	...
	{:else if kidData}
		...
		<div class="bg-white overflow-hidden shadow rounded-lg mb-8">
			<div class="px-4 py-5 sm:p-6">
				...
				<form on:submit|preventDefault={handleWithdrawal} class="space-y-4">
